...
for count, itemLink in enumerate(itemList):
    ...
    metadata = requests.get(baseURL+itemLink, headers=headerAuth).json()
    itemDict = {}
    print(metadata)
    itemDict['itemID'] = itemLink
    itemDict['handle'] = metadata.get('handle')
    print(itemDict)
...
df.to_csv('itemHandleFromLinks_'+dt+'.csv', index=False)
